...
class StartUpProfileVC: CommonTableVC {
    ...
    override func viewDidLoad() {
        ...
        if let theEmail = userEmail {
            params["email"] = theEmail
        } else {
            params["userId"] = StoreKey.me.value!.id
        }
        Helper.with(self).post(url: "/user/profile", params: params) { [weak self] res in
            guard let `self` = self else { return }

            self.alert(message: self.userEmail == nil ?
                "Now to help you get set up, fill out your profile, services & Service photos":
                    "Thanks, your account has been activated. Now let's proceed with setting up your profile")
            ...
            self.parish.setup(res["data"]["parishes"].arrayValue.map { Common(id: 0, name: $0.stringValue) })
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if let vc = segue.destination as? StartUpServicePackageVC {
...
extension StartUpProfileVC: UITextViewDelegate {
...
extension StartUpProfileVC: ImagePickerDelegate {
